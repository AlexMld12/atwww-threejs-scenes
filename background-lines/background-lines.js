/* ────────────────────────────────────────────────────────────────────────────
   Animated topographic contour lines (landonorris.com style). Self-contained,
   dependency-free WebGL2. One fullscreen draw call per instance.

   The lines are ISO-CONTOURS of an animated 3D noise field (time = the 3rd axis,
   so the 2D slice MORPHS IN PLACE — no drift): amber rings form around peaks and
   fuse / split like cells. Smooth, rounded, anti-aliased, amber @ ~5%, on dark.

   ── Placement ──────────────────────────────────────────────────────────────
   By default the script mounts an INDEPENDENT lines canvas into EVERY element
   carrying the attribute  data-lines="background"  — so you can drop the effect
   behind any section, anywhere, as many as you like. Each such element should be
   sized/positioned by you (e.g. a sticky 100vh div); the canvas fills it.

   If NO such element exists and no `mount` is set, it falls back to a single
   fixed, full-page canvas behind the whole page.
...
(function () {
  ...
  var CONFIG = Object.assign({
    ...
    transparent: false,       // false = paints its own dark bg + lines; true = only lines (host bg shows through)
    ...
    zIndex:      0,           // canvas stacking within its container (keep behind that container's content)
    dprCap:      IS_MOBILE ? 1.0 : 1.5,
    renderScale: IS_MOBILE ? 0.7 : 0.9, // render-buffer fraction (subtle bg → sub-res is invisible & faster)
    // Which host elements get a lines canvas. Default = every [data-lines="background"].
    selector:    '[data-lines="background"]',
    mount:       null,        // optional extra id/selector to also mount into
  ...
  // ── Shader source (shared text; compiled per-context) ───────────────────────
  var VERT =
    '#version 300 es\n' +
    'void main(){\n' +
  ...
  function createInstance(canvas) {
    ...
    gl.linkProgram(prog);
    if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) { console.warn('[bg-lines] link:', gl.getProgramInfoLog(prog)); return null; }
    gl.useProgram(prog);
  ...
  // ── Canvas factories ─────────────────────────────────────────────────────────
  function canvasInContainer(el) {
    if (el.__bgLines) return null;                // already initialised
    el.__bgLines = true;
  ...
  // ── Resolve host elements + spin up an instance for each ────────────────────
  function collect() {
    var nodes = [];
    var seen = [];
    function add(list) {
      for (var i = 0; i < list.length; i++) if (seen.indexOf(list[i]) === -1) { seen.push(list[i]); nodes.push(list[i]); }
    }
    if (CONFIG.selector) { try { add(document.querySelectorAll(CONFIG.selector)); } catch (e) {} }
    if (CONFIG.mount) {
      var byId = document.getElementById(CONFIG.mount);
      if (byId) add([byId]);
      else { try { add(document.querySelectorAll(CONFIG.mount)); } catch (e2) {} }
    }
    return nodes;
  }
